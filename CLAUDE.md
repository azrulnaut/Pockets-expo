...
- **MAX button in PurposeGrid** — each row has a MAX button. `handleMax` in `RebalanceModalContent` computes `effectiveRemainder = remainder + signedCurrent` (un-doing the row's existing contribution) so pressing MAX sets remainder to exactly 0. Positive case: sets row to `effectiveRemainder` in `+` mode. Negative case: caps at `currentInAccount` to prevent overdraw. Per-row disabled: `effectiveRemainder === 0` OR (`effectiveRemainder < 0` and `currentInAccount === 0`). Red input text (`#ef4444`) when `-` mode value exceeds `currentInAccount`. `onMaxPress` handler lives in `RebalanceModalContent` and is passed as a prop.
